...
// 追加部分
volatile bool isUpdiMode = false;
#define WRITER_MODE_CHANGE 6
#define ISP_MODE_LED       4
#define UPDI_MODE_LED      5

void setup() {

  // 追加部分
  pinMode(WRITER_MODE_CHANGE, INPUT_PULLUP);
  pinMode(ISP_MODE_LED, OUTPUT);
  pinMode(UPDI_MODE_LED, OUTPUT);
  _delay_ms(10);

  if (digitalRead(WRITER_MODE_CHANGE)) {
    isUpdiMode = true;
    digitalWrite(UPDI_MODE_LED, HIGH);
    // UPDI モードであれば処理終了
    return;
  } else {
    digitalWrite(ISP_MODE_LED, HIGH);
  }
